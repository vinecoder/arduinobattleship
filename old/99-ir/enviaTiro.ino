...
const int botao1 = 4; // O 1º pino de uma sequencia de 5 pinos connectado ao botão
...
void loop() {
  for (int numBotao=0; numBotao<botaoPressionado; numBotao++){
    ...
    if (estadoBotao[numBotao] != ultimoEstadoBotao[numBotao]){
      ...
      irsend.sendSony(codigoIrBotao[numBotao], 32); // Trocar a função sendSony pelas coordenadas X,Y do tiro
